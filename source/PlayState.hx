...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'stage':
				{
						...
						//this was a little idea of mine that i thought would work, nope. crashes the fucking game 
						//iconP1.alpha = 0;
						//iconP2.alpha = 0;
						//healthBar.alpha = 0;
						//healthBarBG.alpha = 0;
						...
						//var healthBarAnim:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('healthBarSwitch'));
						//healthBarAnim.setGraphicSize(Std.int(healthBarAnim.width * 0.9));
						//healthBarAnim.updateHitbox();
						//healthBarAnim.antialiasing = true;
						//healthBarAnim.scrollFactor.set(1.3, 1.3);
						//healthBarAnim.active = false;

						healthBarAnim = new FlxSprite(189, FlxG.height * 0.9 - 91);
						healthBarAnim.frames = Paths.getSparrowAtlas('healthBarSwitch');
						healthBarAnim.animation.addByPrefix('switch', 'poison', 25, false);
						//healthBarAnim.screenCenter(X);
						healthBarAnim.scrollFactor.set();
						//healthBarAnim.animation.play('idle');
						//healthBarAnim.scrollFactor.set(0.8, 0.9);
						//healthBarAnim.scale.set(6, 6);
						//if(dad.curCharacter == 'green-man'){
							add(healthBarAnim);
						//}
			...
			case 'piss':
				{
						defaultCamZoom = 0.9;
						curStage = 'piss';
						var bg:FlxSprite = new FlxSprite(-250, 0).loadGraphic(Paths.image('piss_stage'));
						bg.antialiasing = true;
		...
		if(curStage == 'stage' && dad.curCharacter == 'green-man'){
			//healthBarBG.y = 1000;
			add(healthBarBG);
			health = 1.99;
			healthBarBG.alpha = 0;
		...
		if(curStage == 'stage' && dad.curCharacter == 'green-man'){
			healthBar.createFilledBar(0xFFFF0000, 0xFF9E2ED6);	
		}
		else{
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			switch(daRating)
			{
				case 'shit':
					...
					combo = 0;
					misses++;
					//trust me i tried the not operator, didnt work tho
					if(curStage == 'stage' && dad.curCharacter == 'green-man'){
						health -= 0;
					}
					else{
	...
	*/
	...
	override function beatHit()
	{
		...
		if (dad.curCharacter == 'green-man' && !dad.animation.curAnim.name.startsWith("sing"))
		{
			//if (SONG.notes[Math.floor(curStep / 16)] != null)
			//{
			//fun fact, 3RJ sucks with booleans lol
				if (SONG.notes[Math.floor(curStep / 16)].altAnim){
		...
		{
			dad.playAnim('mic-pickup', true);
		}
		if (curBeat == 1 && curStage == 'stage' && dad.curCharacter == 'green-man')
		{
			healthBarAnim.animation.play('switch');
			new FlxTimer().start(1.95, function(deadTime:FlxTimer)
			{
				...
				//add(iconP1);
				//add(iconP2);
				remove(healthBarAnim);
			});
			healthBarAnim.animation.play('switch');
		}
		//i couldve thrown this in the other if statement but no
		if (curStage == 'stage' && dad.curCharacter == 'green-man' && (curBeat == 204 || curBeat == 205)){
			dad.playAnim('idle-alt');
		}
